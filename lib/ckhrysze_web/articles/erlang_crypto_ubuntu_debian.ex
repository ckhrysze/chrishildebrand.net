defmodule CkhryszeWeb.Articles.ErlangCryptoUbuntuDebian do
  ...
  def render(assigns) do
    ~H"""
    <p>
      As a bit of background, I've had a linux OS on a computer in my home since college, and most of the time its been my
    ...
    <p>
      I still don't have all the details around the root cause, be I'll go over the initial symptom and some
      of the deeper
      ones. The project in question has grown to the point that it no longer makes sense to build it from my machine. The
      project is an Elixir application that uses
      ...
      image doing the building there is the
      <.a href="https://hub.docker.com/_/elixir/">official Elixir docker image</.a>.
      The transition was very smooth until I actually went to run the release on the server. The Erlang process
      immediately crashed, and the erl_crash.dump file included these snippets:
    </p>

    <pre class=" border-gray-500 rounded bg-gray-300 my-4 pt-4">
    ...
    <p>
      The initial round of searching online didn't reveal anything that just worked, so I took a step back and worked
